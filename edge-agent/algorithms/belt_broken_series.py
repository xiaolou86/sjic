...
class BeltBrokenSeriesAlgorithm(BaseAlgorithm):
    """边缘端：皮带表面故障检测算法-级联(Series)检测版本"""
    ...
    def process(self, camera_stream, config_dict, logger, stop_event, on_alert, runtime):
        ...
        try:
            ...
            # 通过 Runtime 抽象加载模型
            logger.info(f"Loading Model for Series via runtime: {model_path}")
            ...
            logger.error(f"Error in belt broken series algorithm: {str(e)}")
            raise
